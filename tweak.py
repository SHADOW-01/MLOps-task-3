...
def train():  
    train_data_dir = '/project/training_set/'
    validation_data_dir = '/project/test_set/'

    save =sys.stdout
    sys.stdout = open("/project/output.txt","w+")
    ...
    checkpoint = ModelCheckpoint("/project/tweak.h5",
...
save = sys.stdout
sys.stdout = open("/project/new_accuracy.txt", "w+")
print(100*accuracy[index])
sys.stdout.close()
sys.stdout = save
save = sys.stdout
sys.stdout = open("/project/best_accuracy.txt","w+")
print("\n\n\n\nMaximum accuracy achieved is : {}%".format(100*accuracy[index]))
print("\n\nHyper-Parameters of the Best-model are : ")
print("No. of Convolutional Layers : {}".format(layer_arr[hyper_parameter[index][4]]))
if (layer_arr[hyper_parameter[index][4]])>3 :
